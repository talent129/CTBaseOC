...
#import "CTJSOCWebController.h"
#import <WebKit/WebKit.h>

@interface CTJSOCWebController ()<WKNavigationDelegate, WKScriptMessageHandler>
...
@implementation CTJSOCWebController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.webView.configuration.userContentController addScriptMessageHandler:self name:@"jsToApp"];
}

- (void)viewDidDisappear:(BOOL)animated {
    [super viewDidDisappear:animated];
    NSLog(@"viewDidDisappear");
    [self.webView.configuration.userContentController removeScriptMessageHandlerForName:@"jsToApp"];
}
...
- (void)viewDidLoad {
    ...
    [self setupUI];
    [self addObserver];
    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@?t=%f", self.urlString, [[NSDate date] timeIntervalSince1970]]]]];
}
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    NSLog(@"%@ - %@ - %@===%@", message, message.body, message.name, message.body[@"name"]);
