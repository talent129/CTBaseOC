...
+ (NSString *)formatMD:(NSDate *)date {
    return [NSString stringWithFormat:@"%ld月%ld日",(unsigned long)[date month], (unsigned long)[date day]];
}
...
+ (NSString *)formatYMDHM:(NSDate *)date {
    return [NSString stringWithFormat:@"%lu-%02lu-%02lu %02lu:%02lu",(unsigned long)[date year],(unsigned long)[date month], (unsigned long)[date day],(unsigned long)[date hour],(unsigned long)[date minute]];
}

+ (NSString *)formatYMDHMS:(NSDate *)date
{
    return [NSString stringWithFormat:@"%lu-%02lu-%02lu %02lu:%02lu:%02lu",(unsigned long)[date year],(unsigned long)[date month], (unsigned long)[date day],(unsigned long)[date hour],(unsigned long)[date minute], (unsigned long)[date second]];
}
...
//modify by hgc 2017年07月31日 之前这里为return [[date lastdayOfMonth] weekOfYear] - [[date begindayOfMonth] weekOfYear] + 1; --错误
+ (NSUInteger)weeksOfMonth:(NSDate *)date {
    return [[date lastdayOfMonth] weekOfYear] - [[date begindayOfMonth] weekOfYearBegin] + 1;
}
...
{
    return [NSDate weekOfYearBegin:self];
}
...
+ (NSUInteger)weekOfYearBegin:(NSDate *)date
{
    ...
    NSDate *lastdate = [date begindayOfMonth];
...
- (BOOL)Yesterday:(NSDate *)anotherDate {
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateComponents *components1 = [calendar components:(NSCalendarUnitYear
                                                          | NSCalendarUnitMonth
                                                          | NSCalendarUnitDay)
...
- (BOOL)isYesterday {
    return [self Yesterday:[NSDate date]];
}
...
+ (NSString *)transformWeekName:(NSString *)originalName {
    if ([originalName isEqualToString:@"星期一"]) return @"周一";
    if ([originalName isEqualToString:@"星期二"]) return @"周二";
    if ([originalName isEqualToString:@"星期三"]) return @"周三";
    if ([originalName isEqualToString:@"星期四"]) return @"周四";
    if ([originalName isEqualToString:@"星期五"]) return @"周五";
    if ([originalName isEqualToString:@"星期六"]) return @"周六";
    if ([originalName isEqualToString:@"星期日"]) return @"周日";
    return originalName;
}
...
+ (NSArray *)getDateWeeksFromToday {
    ...
    for (int i =0; i < 14; i ++) {
        ...
        NSArray *subItemArray = [date weekMonthDayStrings];
        [weeks addObject:subItemArray];
    }
    return weeks;
}
...
- (NSArray <NSString *>*)weekMonthDayStrings {
    ...
    NSDateFormatter *weekFormater = [[NSDateFormatter alloc] init];
    [weekFormater setDateFormat:@"EEEE"];
    ...
    NSString *weekString = [[self class] transformWeekName:[weekFormater stringFromDate:date]];
    if ([self isToday]) {
        weekString = @"今天";
    }
    NSString *monthDayString = [formatter stringFromDate:date];
    [subItemArray addObject:date];
    [subItemArray addObject:monthDayString];
    [subItemArray addObject:weekString];
    return subItemArray;
//        [weeks addObject:subItemArray];
//    }
//    return weeks;
